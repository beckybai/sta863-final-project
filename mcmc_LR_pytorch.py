...
def logpost(beta, data):
    # llx = (likelihood(beta, X_train))
    # wx = sigmoid(torch.mm(data,beta))

    wx = (torch.mm(data,beta))
    wxy = torch.functional.F.softplus(-wx.squeeze()*(y_train*2-1))
    lp = -torch.mean(wxy)
    wx = 100*lp-0.01*torch.mean(0.5*data**2)
...
def train(beta_current):
    ...
    for i in range(total_iter):
        log_prob_curr = logpost(beta_current,X_train)
        propo = proposal_dis()
        log_prob_next = logpost(beta_current+propo,X_train)
        ...
        if(i%200==0) and len(beta_list)>0:
            print('alpha', alpha)
            accurate = predict(X_test,y_test,beta_list[-1])
            accur_list.append(accurate)
            print(i,":",predict(X_test,y_test,beta_list[-1]))
            print(i,":",predict(X_train,y_train,beta_list[-1]))
